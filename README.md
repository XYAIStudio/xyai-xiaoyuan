...
- 番茄钟与轻量心情能量条；桌宠旁短气泡（重连 / 问候 / 番茄钟 / 错误），不刷系统通知
- 对话快捷筹码与口令：拍一拍、喂食、晚安、开始专注；托盘可拍一拍 / 下一阶段

全局快捷键默认：`CmdOrCtrl+Shift+Y` 显示小元，`CmdOrCtrl+Shift+H` 打开当前后端主页，`CmdOrCtrl+Shift+C` 打开对话，`CmdOrCtrl+Shift+T` 切换点击穿透，`CmdOrCtrl+Shift+P` 番茄钟，`CmdOrCtrl+Shift+K` 拍一拍。左键单击桌宠轮换姿态，双击打开对话，右键打开菜单（含 16 表情、「拍一拍 / 喂食 / 晚安」与「锁定姿态」）。托盘可打开小元 / 对话 / 番茄钟 / 设置 / 检查更新。
